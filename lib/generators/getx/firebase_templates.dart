...
''';
  ...
  static Future<void> _updateAuthRepository() async {
    ...
    if (lastImportIndex == -1) lastImportIndex = 0; // Fallback
    ...
    // --- 2. Update Abstract Class ---
    final abstractClassEnd = lines.indexWhere((line) => line.trim().startsWith('}'), lines.indexWhere((l) => l.contains('abstract class AuthRepository')));
  ...
  Future<UserModel?> signInWithGoogle() async {
    try {
      ...
      // Sign in to Firebase to get the user's details
      final UserCredential userCredential = await _firebaseAuth.signInWithCredential(credential);
      final User? firebaseUser = userCredential.user;

      if (firebaseUser != null) {
        // Now, send the social login data to your backend to get your app's user model and token
  ...
  Future<UserModel?> signInWithApple() async {
    try {
      ...
      if (firebaseUser != null) {
        // Now, send the social login data to your backend
        return _socialLoginBackend(
          provider: 'apple',
          providerId: firebaseUser.uid,
  ...
  Future<UserModel> _socialLoginBackend({
    ...
    try {
      // NOTE: You'll need to add a 'social-login' endpoint to your AppUrls.
      // This is a hypothetical endpoint.
      final response = await apiService.post<UserResponseModel>(
        endpoint: '/api/social-login', // Example endpoint
        data: {
          'provider': provider,
...
''';
  ...
  static void _printSocialAuthInstructions() {
    print('\n');
    print('--------------------------------------------------');
    ...
    print('      - You can get your SHA-1 key by running `cd android && ./gradlew signingReport` in your project terminal.');
